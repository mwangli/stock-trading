...
public class DecisionAggregator {
    ...
    public StrategyScore calculateScore(String stockCode, List<MinuteBar> bars,
        ...
        boolean volumeDivergence = volumeCalculator.checkVolumeDivergence(bars, config.getVolumeShrinkThreshold());

        // 计算综合得分
        int totalScore = calculateTotalScore(
                trailingStopTriggered, rsiValue, volumeRatio, bollingerBreakout, volumeDivergence, config);
    ...
    private int calculateTotalScore(boolean trailingStopTriggered, double rsiValue,
            double volumeRatio, boolean bollingerBreakout, boolean volumeDivergence, StrategyConfig config) {
