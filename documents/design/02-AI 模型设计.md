...
AI 模型模块采用分层设计，将数据处理、模型训练、模型服务和持久化等功能解耦。模型管理采用“无文件 IO”方案，所有模型参数直接序列化为字节流并存储在 MongoDB 中。
持久化方式：MongoDB Binary Storage (LstmModelDocument)。
...
- **Service (服务层)**: 核心业务逻辑层。
  - `LstmTrainerService`: 封装了完整的模型训练流程，包括数据获取、预处理、模型训练、评估和保存。
    - `LstmInferenceService`: 负责加载并管理模型推理。模型以二进制字节数组的形式从 MongoDB Binary Storage 动态加载至内存，全程在内存中操作，不依赖任何本地文件系统。推理时，`modelPath` 字段存储 MongoDB 标识符（格式为 `mongo:ID`）。通过 MongoDB Binary Storage 实现，彻底移除本地磁盘文件依赖。
...
- **Storage (存储方式)**: MongoDB Binary Storage，模型参数以二进制字节流存储。
- **Cache (缓存)**: `ModelCacheManager` 负责缓存加载后的模型实例，避免频繁从 MongoDB 读取。模型标识符统一使用 `mongo:ID` 格式。
...
### 2.3. 数据预处理与特征工程 (`LstmDataPreprocessor`)
...
6.  **模型保存**: 训练结束后（或早停触发时），调用 `saveModel` 方法将最佳模型序列化为内存中的字节数组 (`byte[]`) 并保存至 MongoDB。该方法返回一个以 `mongo:` 为前缀的唯一标识符。

## 4. 模型持久化设计（MongoDB Binary Storage）

### 4.1. 核心逻辑

当 `saveModel` 方法被调用时，传入的 `modelIdentifier` (通常为股票代码) 是模型的业务名称。方法返回生成的 MongoDB 唯一标识符（`mongo:ID`）。

1.  **删除旧模型**:
    - 调用 `lstmModelRepository.deleteByModelName(modelIdentifier)`。
    - Spring Data MongoDB 会执行删除操作，移除 `lstm_models` 集合中匹配的文档。
2.  **序列化新模型**:
    - 将 DJL 在内存中构建的模型（包括权重、结构等）和归一化参数直接序列化为二进制字节数组 (`byte[]`)。全程不产生任何本地磁盘文件，不进行任何文件系统 I/O，完全基于 MongoDB Binary Storage 实现。
3.  **保存新模型**:
    - 创建一个新的 `LstmModelDocument` 实例。
    - **`modelName`**: 设置为传入的 `modelIdentifier`。
    - **`params`**: 设置为二进制字节数组（MongoDB Binary Storage）。
    - **`normalizationParams`**: 保存归一化参数的文本内容。
    - 调用 `lstmModelRepository.save(doc)` 将新模型数据写入 MongoDB。

### 4.2. `LstmModelDocument` 实体

`LstmModelDocument` 记录模型业务标识、训练信息、二进制参数与归一化参数，是 MongoDB Binary Storage 的持久化结构。
...
public class LstmModelDocument {
    ...
    // 模型参数与归一化参数的二进制字节数组
    private byte[] params; // 模型参数的二进制字节数组 (byte[]), 由 ModelBinaryCodec 序列化
...
2.  **加载模型**:
    ...
    - 若缓存未命中，调用 `LstmModelRepository.findByModelName(stockCode)` 从 MongoDB 获取模型文档。模型数据以 `byte[]` 形式读取。
- 将模型二进制字节数组直接在内存中作为 `InputStream` 加载，并使用 DJL 的 `model.load(InputStream)` 接口解析。全程不写入磁盘，实现纯内存模型 IO。
    - 将加载后的模型存入缓存。
3.  **数据准备**:
