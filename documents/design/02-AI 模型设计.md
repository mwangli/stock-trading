...
- **Service (服务层)**: 核心业务逻辑层。
  ...
  - `LstmInferenceService`: 负责加载并管理模型推理。模型以二进制字节数组的形式从 MongoDB 动态加载至内存，全程在内存中操作，不依赖任何本地文件系统。推理时，`modelPath` 字段存储 MongoDB 标识符（格式为 `mongo:ID`）。
...
- **Storage (存储方式)**: MongoDB Binary Storage (LstmModelDocument)，模型参数以二进制字节流存储。
#RJ|- **Cache (缓存)**: `ModelCacheManager` 负责缓存加载后的模型实例，避免频繁从 MongoDB 读取。模型标识符统一使用 `mongo:ID` 格式。
...
2.  **序列化新模型**:
    - 将 DJL 在内存中构建的模型（包括权重、结构等）和归一化参数直接序列化为二进制字节数组 (`byte[]`)。全程不产生本地磁盘文件，不进行任何文件系统 I/O。
3.  **保存新模型**:
...
`LstmModelDocument` 记录模型业务标识、训练信息、二进制参数与归一化参数，是 MongoDB 的持久化结构。
...
- 将模型二进制字节数组直接在内存中作为 `InputStream` 加载，并使用 DJL 的 `model.load(InputStream)` 接口解析。全程不写入磁盘。
    - 将加载后的模型存入缓存。
3.  **数据准备**:
