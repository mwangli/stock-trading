...
- **Service (服务层)**: 核心业务逻辑层。
  ...
  - `LstmInferenceService`: 负责加载模型并执行预测。
...
- **Cache (缓存)**: `ModelCacheManager` (可选，后期优化) 负责缓存加载后的模型实例，避免重复IO。
...
6.  **模型保存**: 训练结束后（或早停触发时），调用 `saveModel` 方法将最佳模型持久化。
...
1.  **删除旧模型**:
    ...
    - Spring Data MongoDB 会生成并执行一个 `DELETE` 操作，删除 `lstm_models` 集合中 `modelName` 字段与 `modelIdentifier` 匹配的所有文档。
2.  **打包新模型**:
    - 将 DJL 在本地文件系统生成的模型文件（如 `.pt` 权重文件、`synset.txt` 等）和归一化参数文件打包成一个 ZIP 压缩字节流。
3.  **保存新模型**:
...
2.  **加载模型**:
    ...
    - 若缓存未命中，调用 `LstmModelRepository.findById(stockCode)` 从 MongoDB 获取模型文档。
    - 解压模型 ZIP 字节流到内存（或临时文件），使用 DJL 的 `model.load()` 方法加载模型。
    - 将加载后的模型存入缓存。
3.  **数据准备**:
