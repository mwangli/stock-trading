...
121#PQ|    - **最后同步日期 (lastSyncDate)**: 通过 `PriceRepository` 查询每支股票在数据库中存在的最新日期。
122#QW|    - **自动范围计算**: 
123#ZX|      - **增量同步**: 计算 `[lastSyncDate + 1, today]` 缺失范围进行抓取。
124#VH|      - **全量初始化**: 若数据库无记录，自动从系统预设日期 (如 10 年前) 开始同步。
125#TH|    - **断点续传**: 同步过程中若发生中断，下次执行时会从 `lastSyncDate` 处自动继续，无需重新同步历史数据。
126#MT|    - **空洞填补**: 定时任务会通过 `findByCodeAndDateBetween` 检查特定时间段内的数据连续性，自动补全缺失的交易日数据。
127#QJ|    - **减少开销**: 仅向证券平台请求必要的时间段数据，极大降低 API 调用额度和网络流量负载。
128#MS|    - **并发同步**: 使用 `CompletableFuture` 实现多支股票并行执行增量同步，互不干扰。
