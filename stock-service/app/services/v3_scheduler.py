...
- Data Collection (MOD-001)
- Sentiment Analysis (MOD-002)
- LSTM Prediction (MOD-003)
- Stock Ranking (MOD-004)
...
class V3Scheduler:
    """
    V3.0 Scheduler for all Python scheduled tasks
    ...
    def start(self):
        ...
        if self.is_running:
            logger.warning("V3 Scheduler is already running")
            return

        logger.info("Starting V3.0 scheduler with all modules...")
    ...
    def stop(self):
        ...
        logger.info("Stopping V3.0 scheduler...")
    ...
    def _log_scheduled_jobs(self):
        ...
        logger.info(f"V3.0 Scheduler: {len(jobs)} jobs scheduled")
        for job in jobs:
    ...
    def _log_task_end(self, log_id: int, status: str, records_processed: int = 0, 
        ...
        db = MySQLSessionLocal()
        try:
            log = db.query(TaskExecutionLogModel).filter(
                TaskExecutionLogModel.id == log_id
            ).first()
            if log:
                log.status = status
    ...
    def _add_data_collection_jobs(self):
        # Stock list sync - Daily at 09:00
        ...
        # Historical data sync - Daily at 15:30
        self.scheduler.add_job(
        ...
        # Real-time quotes - Every minute during trading
        self.scheduler.add_job(
    ...
    def _add_sentiment_analysis_jobs(self):
        self.scheduler.add_job(
            ...
            trigger=CronTrigger.from_crontab(
                settings.SENTIMENT_ANALYSIS_CRON.replace('?', '*')
            ),
            ...
            for stock in stocks:
                try:
                    ...
                    sentiment_record = StockSentimentModel(
                        ...
                        positive_ratio=market_sentiment.get("positive_count", 0) / max(market_sentiment.get("total_count", 1), 1),
                        negative_ratio=market_sentiment.get("negative_count", 0) / max(market_sentiment.get("total_count", 1), 1),
                        neutral_ratio=market_sentiment.get("neutral_count", 0) / max(market_sentiment.get("total_count", 1), 1),
    ...
    async def _fetch_stock_news(self, stock_code: str) -> list:
        ...
        # Placeholder - implement actual news fetching from AKShare or other source
        # This should integrate with the existing news API
        try:
            from app.api.data_collection import get_stock_news
            news = await get_stock_news(stock_code)
    ...
    def _add_lstm_prediction_jobs(self):
        self.scheduler.add_job(
            ...
            trigger=CronTrigger.from_crontab(
                settings.LSTM_PREDICTION_CRON.replace('?', '*')
            ),
    ...
    async def _lstm_prediction_job(self):
        ...
        try:
            ...
            stocks = db.query(StockInfoModel).filter(
                StockInfoModel.deleted == '0',
                StockInfoModel.is_tradable == 1
            ).limit(50).all()  # Limit for prediction
            ...
            for stock in stocks:
                try:
                    ...
                    from app.services.data_collection_service import data_collection_service
                    historical_data = await data_collection_service.get_historical_data(
                        stock.code, days=60
                    )
                    ...
                    logger.error(f"[MOD-002] Failed to predict {stock.code}: {e}")
                    continue
    ...
    def _add_stock_ranking_jobs(self):
        self.scheduler.add_job(
            ...
            trigger=CronTrigger.from_crontab(
                settings.STOCK_RANKING_CRON.replace('?', '*')
            ),
    ...
    async def _stock_ranking_job(self):
        ...
        try:
            ...
            sentiments = db.query(StockSentimentModel).filter(
                StockSentimentModel.analyze_date >= today
            ).all()
            sentiment_map = {s.stock_code: s for s in sentiments}
            
            predictions = db.query(StockPredictionModel).filter(
                StockPredictionModel.predict_date >= today
            ).all()
            ...
            for stock in stocks:
                try:
                    ...
                    rankings.append({
                        ...
                        "momentum_score": 50 + (prediction.confidence * 50) if prediction and prediction.confidence else 50,
                        "valuation_score": 50,  # Placeholder
...
# Singleton instance
v3_scheduler = V3Scheduler()


def init_scheduler():
    """Initialize and start the V3.0 scheduler"""
    v3_scheduler.start()
