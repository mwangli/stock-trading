    ...
    private void saleStock(TradingRecord record, CountDownLatch countDownLatch) {
        ...
        while (!finished) {
            try {
                ...
                log.info("当前股票[{}-{}],最新价格为:{}，平均价格为:{}，期望卖出价格为：{}, 已统计次数为:{}", record.getName(), record.getCode(), String.format("%.2f", nowPrice), String.format("%.2f", priceAvg), String.format("%.2f", expectedPrice), priceCount);
