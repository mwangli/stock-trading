...
- **采集策略**: 每页 100 条、最大 1 页，单股票单来源（新闻/公告）各 100 条
...
| 参数 | 值 | 说明 |
|------|-----|------|
| 每页条数 | 100 | 接口 maxcount |
| 最大页数 | 1 | 单股票单来源（新闻/公告）各 100 条 |
...
| 优化项 | 实现 |
|--------|------|
| 方案 A 批量预查 | 每页 1 次 findExternalIdsByStockCodeAndExternalIdIn（按 stockCode+externalId 联合判重），100 次 exists → 1 次 in 查询 |
| 多线程 | 6 线程并行采集股票 |
| 批量写入 | 每 100 条 BulkOperations.insert，失败回退逐条 |
| 请求间隔 | 150ms/条 |
