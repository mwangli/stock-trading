...
{
    _id: String,
    externalId: String,      // 证券平台新闻 ID，去重
    title: String,
    content: String,
    stockCode: String,
    source: String,
...
```
**索引**: `externalId` 唯一索引，`stockCode + publishTime` 复合索引
...
| 优化项 | 实现 |
|--------|------|
| 方案 A 批量预查 | 每页 1 次 findExternalIdsByExternalIdIn（projection 仅 externalId），100 次 exists → 1 次 in 查询 |
| 多线程 | 6 线程并行采集股票 |
...
全量与增量使用同一逻辑，接口按时间倒序返回，已入库通过 externalId 跳过。
...
### Q7: 新闻批量预查慢，如何验证索引？
A: stock_news 集合应有 `idx_externalId` 唯一索引。在 MongoDB Shell 执行：
```javascript
db.stock_news.getIndexes()           // 查看索引
db.stock_news.find({externalId:{$in:["id1","id2"]}}).explain("executionStats")  // 验证查询计划
```
若索引缺失，重启应用（auto-index-creation: true 会创建）或手动执行 `db.stock_news.createIndex({externalId:1},{unique:true,name:"idx_externalId"})`。