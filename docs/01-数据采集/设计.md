...
- ✅ **新闻采集**: 已实现，采集新闻+公告，支持全量/增量、多线程、批量写入
- ✅ **证券平台 403 处理**: 支持 HTTP/SOCKS5 代理配置，解决云服务器 IP 被拦截问题
...
#### 新闻采集 (DataSyncScheduler / NewsStartupListener)
- **位置**: `support/DataSyncScheduler.java`、`support/NewsStartupListener.java`
- **采集策略**: 每页 50 条、最大 1 页，单股票单来源（新闻/公告）各 50 条
- **性能**: 6 线程并行、每 50 条批量写入、150ms 请求间隔
- **任务**:
  - **全量初始化**: 启动监听器 `app.startup.news-sync.enabled=true`
  - **全量周维护**: `newsFullSync` 每周日 6:00（默认关闭）
  - **每日增量**: `newsDailySync` 每交易日 6:00（默认关闭）
- **采集来源**: 新闻 menu_id=20002、公告 menu_id=20001
...
获取所有股票列表
    ↓
8 线程并行处理每支股票（平台接口默认返回最近约 500 条）
    ↓
...
```javascript
{
    _id: ObjectId,
    code: String,           // 股票代码
    stockName: String,       // 股票名称，冗余存储便于展示
    date: LocalDate,        // 交易日期
...
{
    _id: String,
    externalId: String,      // 证券平台新闻 ID，与 stockCode 联合唯一（同一条新闻可关联多只股票）
    title: String,
    content: String,
    stockCode: String,
    stockName: String,      // 股票名称，冗余存储便于展示
    source: String,
    category: String,       // 新闻/公告
    publishTime: LocalDateTime,
    createTime: LocalDateTime
}
```
**索引**: `idx_stock_external` 联合唯一 (stockCode, externalId)，`idx_stock_publish` 复合 (stockCode, publishTime)
...
- **现象**: 本地请求正常，云服务器（如 124.220.36.95）请求 `weixin.citicsinfo.com` 返回 403
- **根因**: 平台对云服务器/数据中心 IP 做访问控制（WAF/CDN），仅放行住宅/白名单 IP
- **解决方案**（按推荐顺序）:
  1. **HTTP/SOCKS5 代理**（推荐）: 通过本地或可访问代理发出请求，项目已支持 `app.securities.proxy` 配置
  2. **SSH 隧道**: 本地启动代理，`ssh -R 8888:127.0.0.1:8888 user@远程` 暴露到远程
  3. **住宅代理服务**: 购买 residential 代理（如 Luminati、SmartProxy）
  4. **联系平台白名单**: 正式业务可申请 IP 白名单
  5. **采集与业务分离**: 采集部署在本地，业务在云端消费已同步数据
- **配置示例**（`application.yml`）:
  ...
  app:
    securities:
      proxy:
        enabled: true
        host: 127.0.0.1   # 或 host.docker.internal（Docker）
        port: 1080
        type: HTTP        # 或 SOCKS5
  ```
...
**平台接口约束**：不支持单日区间，默认返回最近约 500 条。

**实现概览**：

| 场景 | 位置 | 并行 | 流程 |
|------|------|------|------|
| 冷启动全量 | ApplicationStartupListener | 8 线程 | existsByCodeAndDate → getHistoryPrices → saveStockPrices(upsert) |
| 每日增量 | DataSyncScheduler.syncDailyStockData | 8 线程 | getHistoryPrices → findTopByCodeOrderByDateDesc → 过滤 → saveStockPrices(upsert) |

**优化状态**：

| 优化项 | 状态 | 说明 |
|--------|------|------|
| 每日增量并行 | ✅ 已实现 | 8 线程固定线程池 |
| 冷启动线程池 | ✅ 已实现 | 8 线程固定线程池 |
| 按日期拉取 | ❌ 不适用 | 平台不支持单日区间 |
| findTop 批量化 | ⏳ 待实现 | 1 次聚合替代 5000 次单股查询 |
| 冷启动 exists 批量化 | ⏳ 待实现 | 1 次查询替代 5000 次 exists |

**写入层**：BulkOperations.upsert 已最优，无需预查。

### 12.3 新闻采集

**状态**: ✅ **已实现**

**位置**: `service/StockNewsService`, `support/DataSyncScheduler`, `support/NewsStartupListener`
...
|------|-----|------|
| 每页条数 | 50 | 降低采集负担与限流风险 |
| 最大页数 | 1 | 单股票单来源（新闻/公告）各 50 条 |
| 采集来源 | menu_id 20002/20001 | 新闻、公告 |

**性能优化**：

| 优化项 | 实现 |
|--------|------|
| 方案 A 批量预查 | 每页 1 次 findExternalIdsByStockCodeAndExternalIdIn（按 stockCode+externalId 联合判重），50 次 exists → 1 次 in 查询 |
| 多线程 | 6 线程并行采集股票 |
| 批量写入 | 每 50 条 BulkOperations.insert，失败回退逐条 |
| 请求间隔 | 150ms/条 |
...
| 场景 | 触发 | 任务 |
|------|------|------|
| 全量初始化 | `app.startup.news-sync.enabled=true` | 启动后异步执行 |
| 全量周维护 | newsFullSync | 每周日 6:00（默认关闭） |
| 每日增量 | newsDailySync | 每交易日 6:00（默认关闭） |

全量与增量使用同一逻辑，接口按时间倒序返回，已入库通过 (stockCode, externalId) 联合判重跳过。
...
### Q4: 新闻采集何时实现？
A: 已实现，通过 `app.startup.news-sync.enabled=true` 启动全量采集，或启用 `newsFullSync`/`newsDailySync` 定时任务。

### Q5: 云服务器请求证券平台返回 403 怎么办？
A: 证券平台对云服务器 IP 做了限制。优先配置 HTTP/SOCKS5 代理（见 9.5 节），或采用采集与业务分离部署。

### Q6: 如何单独构建此模块？
A: 执行 `cd backend && mvn clean package -DskipTests`（本项目为单体应用，在 backend 目录构建）。

### Q7: 新闻批量预查慢，如何验证索引？
A: stock_news 集合应有 `idx_stock_external` 联合唯一索引 (stockCode, externalId)。在 MongoDB Shell 执行：
```javascript
db.stock_news.getIndexes()           // 查看索引
db.stock_news.find({stockCode:"000001",externalId:{$in:["id1","id2"]}}).explain("executionStats")  // 验证查询计划
```
若曾使用旧版 `externalId` 单一唯一索引，需先删除再让应用创建新索引：
```javascript
db.stock_news.dropIndex("externalId")   // 或 "idx_externalId"，视库中实际名称
```
重启应用后会自动创建 `idx_stock_external`。