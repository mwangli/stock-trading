...
- ✅ **新增滑块验证码破解**: CaptchaService 实现券商平台登录时的滑块验证码自动识别与破解
...
## 14. 滑块验证码破解 (CaptchaService)

### 14.1 模块概述

滑块验证码破解模块负责在券商平台登录时自动识别并通过滑块验证码，实现无人值守的自动化登录流程。
...
**流程说明**：
1. **发送登录请求**：向券商平台发送用户名密码登录请求
2. **验证码检测**：判断返回是否包含验证码挑战
3. **图片获取**：获取滑块验证码的背景图和滑块小块图
4. **缺口定位**：通过图像识别算法计算滑块需要移动的距离
5. **轨迹生成**：生成模拟人类滑动行为的轨迹曲线
6. **验证提交**：将滑动结果提交给券商平台验证
7. **重试机制**：验证失败时支持自动重试
...
| 组件 | 位置 | 职责 |
|------|------|------|
| CaptchaService | execution/CaptchaService.java | 滑块验证码图像识别和破解 |
| ZXRequestUtils | execution/ZXRequestUtils.java | 新增 loginWithCaptcha() 带验证码登录方法 |
| ZXBrokerAdapter | execution/ZXBrokerAdapter.java | 新增 login() 券商平台登录方法 |

### 14.4 图像识别算法

**算法流程**：
1. **灰度转换**：将彩色验证码图片转换为灰度图，减少计算复杂度
2. **模板匹配**：在背景图中滑动滑块模板，计算每个位置的像素差异度
3. **位置确定**：找出差异度最小的位置，即为滑块缺口位置
...
```
tradingExecutor/execution/
├── BrokerAdapter.java       # 券商适配接口
├── ZXBrokerAdapter.java    # 中信证券实现
├── ZXRequestUtils.java     # 请求工具类 (已扩展)
├── MockBrokerAdapter.java  # 模拟券商实现
└── CaptchaService.java     # 滑块验证码破解服务 (新增)
```
